define([
  'hbs!../templates/BillingWorkflowWithAnimation.hbs',
  '../actions/BwfWorkflowAction',
  '../modules/step-pop/actions/BwfStepAction',
  '../modules/node-pop/actions/BwfNodeAction',
  'i18n!../i18n/billingNodeSubWorkflow',
  'i18n!tp/i18n/tp.i18n',
  'tp/components/tpUtil',
  '../components/DrawControl',
  '../components/BwfWorkflowCanvasDataProto',
  'tp/components/topo-contextmenu',
  'text!../const/drawMode.json',
  'text!../const/nodePathType.json',
  'text!../const/animationMode.json'
], function(template, bwfWorkflowAction, stepAction, bwfNodeAction, i18n, tpI18n, tpUtil, drawControl,
  bwfWorkflowCanvasDataProto, Contextmenu, drawMode, nodePathType, animationMode) {
  return fish.View.extend({
    template: template,
    serialize: fish.extend(i18n, tpI18n),
    events: {
      'click #js-workflow-new-node': 'newNode',
      'combobox:change .js-combobox-workflow-node': 'locateNode',
      'click .js-copy-workflow': 'copyWorkflow'
    },
    ...
    copyWorkflow: function() {
      fish.popupView({
        url: this.modulePath + 'modules/copy-pop/views/CopyPopView',
        width: '45%',
        close: function(data) {
          if (data) {
            var copyData = {
            ...
            copyData.workflowName = data;
            bwfWorkflowAction.copyBwfWorkflow(copyData, function(returnData) {
              if (returnData) {
                var newRowData = {
                  workflowId: returnData.workflowId,
                  workflowName: returnData.workflowName,
                  workflowType: returnData.workflowType
                };
                // 把新拷贝的数据添加到列表中
                this.options.ratingView.trigger('addWorkflowListAfterCopy', newRowData);
                this.$grid = this.parentView.parentView.$el.find('.js-grid-node');
                if (this.$grid.length > 0) {
                  this.$grid.grid('addRowData', newRowData);
                  this.$grid.grid('setSelection', newRowData);
                }
                fish.toast('success', i18n.COPY_SUCCESS);
              }
            }.bind(this));
          }
        }.bind(this)
    ...
    locateNode: function(event) {
      ...
      var canvas = document.getElementById('js-billing-workflow-chart');
    ...
    constructBwfWorkflowGroupNode: function(bwfWorkflow) {
      var bwfWorkflowGroupNode = bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfWorkflowGroup,
      ...
      for (let bwfNode of bwfWorkflow.bwfNodeList) {
        let bwfNodeGroupNode = bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfNodeGroup, new Path2D(),
          bwfNode, 'nodeName', bwfNode.nodeId);
        bwfNodeGroupNode.children.push(bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfRealNodeName,
          new Path2D(), {}, '', bwfNode.nodeId));
        if (!bwfNode.bwfStepList) {
          bwfNode.bwfStepList = [];
        }
        for (let bwfStep of bwfNode.bwfStepList) {
          let bwfStepGroupNode = bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfStepGroup, new Path2D(),
            bwfStep, 'sortRuleName', bwfStep.stepId);
          bwfStepGroupNode.children.push(bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfStepName,
            new Path2D(), {}, '', bwfStep.stepId));
          if (bwfStep.outputNodeId) {
            bwfStepGroupNode.children.push(bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfReferedNodeName,
              new Path2D(), {}, '', bwfStep.stepId));
          }
          bwfStepGroupNode.children.push(bwfWorkflowCanvasDataProto.constructor(this.nodePathType.bwfStepSymbol,
            new Path2D(), {}, '', bwfStep.stepId));
